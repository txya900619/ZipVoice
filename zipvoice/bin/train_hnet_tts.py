...
def run(rank, world_size, args):
    ...
    def lr_modulation(n_gpt: float = 4.6):  # https://arxiv.org/pdf/2507.07955#page=35
        n_prod_ratio = params.n_compression[::-1]
        d_ratio = [model_config.d_model[-1] / d for d in model_config.d_model]
        return [
            (4.5 * n_frac * d_frac) ** 0.5
            for n_frac, d_frac in zip(n_prod_ratio, d_ratio)
        ]

    def split_params_by_hierachy(m: HNetTTS) -> list[list[nn.Parameter]]:
    ...
    param_groups = [
        dict(params=plist, lr=params.base_lr * λˢ)
        for (plist, λˢ) in zip(split_params_by_hierachy(model), lambda_s)
    ]

    optimizer = torch.optim.AdamW(
        param_groups, betas=(0.9, 0.95), weight_decay=0.01, lr=params.base_lr
    )

    # set scheduler
    def wsd(step: int, end: int):
        pct = step / end
        return pct * 10 if pct < 0.1 else (1 if pct < 0.9 else (1 - pct) * 10)

    get_lr_mult = functools.partial(wsd, end=params.num_iters)  # if is epoch how?
