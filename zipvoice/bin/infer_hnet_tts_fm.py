...
def generate_sentence(
    ...
    tokens = NJT(torch.tensor(tokens)).to(device)
    prompt_tokens = NJT(torch.tensor(prompt_tokens)).to(device)
    prompt_features = NJT(prompt_features).to(device)

    min_len = int(
        prompt_features[0].shape[0]
        / prompt_tokens[0].shape[0]
        * tokens[0].shape[0]
        * 0.5
    )
    ...
    print(pred_features.values().shape)

    pred_features = pred_features.values().unsqueeze(0)

    # Postprocess predicted features
    pred_features = pred_features.permute(0, 2, 1).float() / feat_scale  # (B, C, T)

    # import matplotlib.pyplot as plt

    # plt.figure(figsize=(10, 5))
    # plt.imshow(
    #     pred_features[0, :, :].cpu().numpy(),
    #     origin="lower",
    #     aspect="auto",
    # )
    # plt.savefig("mel_features.png")
    # plt.close()
...
def main():
    ...
    feature_extractor = VocosFbank()
    params.sampling_rate = feature_extractor.config.sampling_rate
