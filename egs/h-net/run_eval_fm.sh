...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
      ...
      for testset in librispeech_pc; do 
        ...
        python3 -m zipvoice.bin.infer_hnet_tts_fm \
                --model-dir exp/hnet_libritts_fm_lr_5e-4_N2 \
                --tokenizer libritts \
                --feat-scale 0.4343 \
                --num-step 16 \
                --guidance-scale 0.0 \
                --t-shift 0.5 \
                --checkpoint-name checkpoint-100000.pt \
                --test-list ${test_tsv} \
                --res-dir ${results_dir}/${testset}
      done
fi
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
      ...
      wav_path=${results_dir}/librispeech_pc
      test_tsv=${download_dir}/librispeech_pc_testset/test.tsv
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
      ...
      model_path=${download_dir}/tts_eval_models
      wav_path=${results_dir}/seedtts_en
      test_tsv=${download_dir}/seedtts_testset/en/test.tsv
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
      ...
      model_path=${download_dir}/tts_eval_models
      wav_path=${results_dir}/seedtts_zh
      test_tsv=${download_dir}/seedtts_testset/zh/test.tsv
