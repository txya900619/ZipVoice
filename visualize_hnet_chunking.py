...
def visualize_chunking(
    ...
    with torch.no_grad():
        x_flat, extra = hnet_backbone(x_flat, cu_s, msl)
    ...
    if len(all_layers_data) > 0:
        ...
        text_length = text_condition.unbind()[sample_idx].shape[0]
...
def main():
    ...
    mels_input = NJT(
        [
            torch.cat((t_i, model.mel_bos, m_i[:-1]), dim=0)
